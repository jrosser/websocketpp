...
# DESCRIPTION
#
#   Test for Random library from the Boost C++ libraries. The macro requires
...
[
	...
	AS_HELP_STRING([--with-boost-random@<:@=special-lib@:>@],
                   [use the Random library from boost - it is possible to specify a certain library for the linker
                        e.g. --with-boost-random=boost_random-gcc-mt-d-1_33_1 ]),
        [
	...
	if test "x$want_boost" = "xyes"; then
        ...
        AC_CACHE_CHECK(whether the Boost::Random library is available,
					   ax_cv_boost_random,
        [AC_LANG_PUSH([C++])
			 AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[@%:@include <boost/nondet_random.hpp>
												]],
                                   [[boost::random_device r(); return 0;]])],
		...
		if test "x$ax_cv_boost_random" = "xyes"; then
            ...
            if test "x$ax_boost_user_random_lib" = "x"; then
                for libextension in `ls $BOOSTLIBDIR/libboost_random*.so* $BOOSTLIBDIR/libboost_random*.a* 2>/dev/null | sed 's,.*/,,' | sed -e 's;^lib\(boost_random.*\)\.so.*$;\1;' -e 's;^lib\(boost_random.*\)\.a*$;\1;'` ; do
                     ax_lib=${libextension}
                ...
                for libextension in `ls $BOOSTLIBDIR/boost_random*.{dll,a}* 2>/dev/null | sed 's,.*/,,' | sed -e 's;^\(boost_random.*\)\.dll.*$;\1;' -e 's;^\(boost_random.*\)\.a*$;\1;'` ; do
                     ax_lib=${libextension}
				    AC_CHECK_LIB($ax_lib, exit,
                                 [BOOST_RANDOM_LIB="-l$ax_lib"; AC_SUBST(BOOST_RANDOM_LIB) link_random="yes"; break],
                                 [link_random="no"])
